...
workflow {
    ...
    if (params.adapter_trimming) {
        ...
        qc_post_alignment(star_alignment.out.bam_files)

        // Step 6: RSEM Transcript Quantification
        rsem_transcript_quantification(star_alignment.out.bam_files, star_and_RSEM_index_building.out.rsem_index.collect())
        ...
        // Step 5: QC after alignment (no adapter trimming)
        qc_post_alignment(star_alignment_no_adapter_trimming.out.bam_files)

        // Step 6: RSEM Transcript Quantification (no adapter trimming)
        rsem_transcript_quantification(star_alignment_no_adapter_trimming.out.bam_files, star_and_RSEM_index_building.out.rsem_index.collect())
    }
}